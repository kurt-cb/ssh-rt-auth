...
- **Spawned by:** `msshd` at startup as a direct subprocess (via
  `asyncio.create_subprocess_exec`). The wrapper supervises the child
  itself; there is **no separate systemd unit** for the inner sshd
  (see § 12.2 for the rationale).
...
```
# Hermetic inner sshd_config — generated by msshd.
...
ExecStart=/usr/sbin/msshd --config /etc/ssh-rt-auth/wrapper.yaml
Restart=on-failure
RestartSec=5
...
### 12.2 Inner sshd lifecycle — wrapper-supervised, NOT systemd-managed

**This section originally proposed a separate
`ssh-rt-inner-sshd.service` systemd unit with
`BindsTo=msshd.service`. That design was dropped in Phase 1B
implementation — see [dropped-ideas.md § Systemd-managed inner sshd](ssh-rt-auth-dropped-ideas.md).**

The implemented approach: the wrapper spawns `sshd -D` as a
subprocess via `asyncio.create_subprocess_exec` and supervises it
directly. The wrapper:

- Allocates the inner sshd's listen port dynamically from
  `inner.port_range` at startup (default `49152-65535`). A
  systemd-managed sshd would have required a fixed `Port` in the
  unit file, contradicting the runtime-allocation design.
- Renders the per-startup hermetic config (with the chosen port
  baked in), writes the host key if absent, drops the user-CA
  pubkey, then spawns `/usr/sbin/sshd -D -f <config>`.
- Waits for the inner port to bind (TCP-poll loop) before declaring
  ready and accepting outer connections.
- On shutdown: SIGTERM the child; SIGKILL after a configurable grace
  window if the child hasn't exited.

The actual implementation lives in
[python/src/sshrt/msshd/inner.py](../python/src/sshrt/msshd/inner.py)
in `class InnerSshd`. The `systemd/` directory in the repo therefore
ships exactly one unit (`msshd.service`); inner sshd has no unit of
its own.
