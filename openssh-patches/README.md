...
| 6 | `enforce-AuthorizationCert-force-command.patch` | Override exec / shell request to the cert's `mssh-force-command` extension when present. Similar shape to `ForceCommand` but cert-driven, not config-driven. | ~50 |
