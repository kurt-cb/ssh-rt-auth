...
# Path to the sibling ssh-rt-auth-openssh repo (where the patches live).
_OPENSSH_REPO = Path(__file__).resolve().parents[3] / 'ssh-rt-auth-openssh'
...
def test_openssh_patches_end_to_end(request):
    ...
    if not _OPENSSH_REPO.exists():
        pytest.skip(f'Sibling repo {_OPENSSH_REPO} not present — '
                    'this test requires the ssh-rt-auth-openssh checkout.')
    ...
    src = _OPENSSH_REPO / 'openssh-portable'
    if not src.exists():
