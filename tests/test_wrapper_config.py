"""Tests for wrapper.python.config."""
...
from wrapper.python.config import (MODE_ENFORCE, MODE_FALLBACK, WrapperConfig)
...
def test_example_yaml_loads(tmp_path):
    """The shipped wrapper/config/wrapper.yaml.example must load
    cleanly. Catches drift between the example and the schema."""
    example = (Path(__file__).resolve().parent.parent
               / 'wrapper' / 'config' / 'wrapper.yaml.example')
