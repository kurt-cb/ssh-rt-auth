...
def loopback_env(tmp_path):
    ...
    from ca.cert_minter import bootstrap_ca, issue_client_cert, load_certificate, load_private_key
    from ca.enrollment import Enrollment, KeyBinding
    ...
    ca_proc = subprocess.Popen(
        [sys.executable, '-m', 'ca.server', '--config', str(cfg_path)],
...
def test_loopback_full_ssh_authorization(loopback_env, tmp_path):
    ...
    from shim.shim import Shim
    from server.ssh_server import _handle_session, make_server_factory
