...
| `test_matrix.py` | For every (user, server) in the random scenario, ssh from `sshrt-lxc-u1` to that server and verify the outcome matches `user.allowed_hosts`. Plus a "from a different source container" pass to confirm the decision is independent of which box you're sshing from. |
